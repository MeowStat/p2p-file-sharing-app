...
def Send_list(trackersocket,peer_list):
    ...
    for peerip, peerport in peer_list:
        ...
        response_ip = trackersocket.recv(1024)
        print(f"Tracker response after IP: {response_ip.decode('utf-8')}")  # Hiển thị phản hồi sau khi gửi IP

        trackersocket.send(str(peerport).encode('utf-8'))
        response_port = trackersocket.recv(1024)
        print(f"Tracker response after port: {response_port.decode('utf-8')}")  # Hiển thị phản hồi sau khi gửi IP
